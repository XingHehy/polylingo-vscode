...
import { getSourceLanguage, getTargetLanguage } from '../core/config';
...
type EditorState = EditorLoadingState | EditorResultState;
...
type SidebarTranslateHandler = (text: string, sourceLanguage: string, targetLanguage: string) => Promise<TranslateResult>;
...
export class ResultPresenter implements vscode.HoverProvider, vscode.WebviewViewProvider, vscode.Disposable {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    this.disposables.push(view.webview.onDidReceiveMessage(async (message) => {
      ...
      if (message?.type === 'translate' && this.sidebarTranslateHandler && !this.sidebarBusy) {
        ...
        this.renderSidebar();
        try {
          const result = await this.sidebarTranslateHandler(text, this.sidebarSourceLanguage, this.sidebarTargetLanguage);
  ...
  private renderSidebar(): void {
    ...
    const original = this.lastOriginal;
